...
def evaluate(opt, args):
    ...
    if args.verbose is True:
        ...
        datasets = tqdm.tqdm(
            opt.Eval.products, desc='Expr - ' + method,
            total=len(opt.Eval.datasets), position=0,
    ...
    for dataset in datasets:
        ...
        for i, sample in samples:
            ...
            threshold_Dice_i = np.zeros(len(Thresholds))

            for j, threshold in enumerate(Thresholds):
                threshold_Pr[j], threshold_Rec[j], threshold_Spe[j], threshold_Dice_i[j], threshold_F[j], threshold_IoU[j] = Fmeasure_calu(
            ...
            threshold_Dice[i, :] = threshold_Dice_i
